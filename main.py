...
MESSAGES = {
    ...
    "confirme_au_bureau":             "🏪 {name}، طلبك جاهز للاستلام!\n\nرقم التتبع: *{tracking}*\n_Rouknadz_",
...
async def test_send(request: Request):
    try:
        ...
        body = {}
    phone    = body.get("phone", "0773219583")
    state    = body.get("state", "sortie_en_livraison")
    name     = body.get("name", "Ahmed")
    tracking = body.get("tracking", "ZR-TEST-001")
    product  = body.get("product", "Strobe LED Intelligent")
